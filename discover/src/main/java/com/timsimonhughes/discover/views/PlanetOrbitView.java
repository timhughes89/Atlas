package com.timsimonhughes.discover.views;
...
import com.timsimonhughes.discover.R;
...
public class PlanetOrbitView extends View implements OnClickListener, OnLongClickListener {
    ...
    private static final int numSteps = 600;             // number animation steps around orbit
    ...
    public boolean showLabels = true;              // Whether to show planet labels
...
//        Define the planet as circular shape
    ...
    protected void onSizeChanged(int width, int height, int oldw, int oldh) {
        ...
        for (int i = 0; i < numObjects; i++) {
            ...
            c2[i] = direction * 2 * Math.PI * Math.sqrt(1 - epsilon[i] * epsilon[i]) * dt * (pixelScale * a[i]) * (pixelScale * a[i]) / period[i];
    ...
    public void startAnimation() {
        ...
        PlanetOrbitView.mState = RUNNING;
        PlanetOrbitView.isAnimating = true;

        new Thread(() -> {
            while (PlanetOrbitView.mState == RUNNING && PlanetOrbitView.isAnimating) {
    ...
    private void drawBackground(Paint paint, Canvas canvas) {
        ...
        if (showOrbits) {
            ...
            paint.setStrokeWidth(ORBIT_STROKE_WIDTH);
    ...
    public void startLooper() {
        if (!isAnimating) {
            String stringMessage = "Long-press to toggle motion on/off";
            Toast.makeText(this.getContext(), stringMessage, Toast.LENGTH_LONG).show();
        }
    }
    ...
    public boolean onLongClick(View v) {
        String longPressMessage = "Long-press toggles planet motion on/off";
        ...
        if (showToast2) {
            Toast.makeText(this.getContext(), longPressMessage, Toast.LENGTH_LONG).show();
    ...
    public void onClick(View v) {
        String shortPressMessage = "Short-press toggles orbit visibility";
        showOrbits = !showOrbits;
        if (showToast1)
            Toast.makeText(this.getContext(), shortPressMessage, Toast.LENGTH_LONG).show();
